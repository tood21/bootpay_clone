    ...
    return (
        <Container>
            <TapMenu>
                {
                    tapMenu.map((menu) => (
                        <StyledButton active={menu.value === tapFlag} key={menu.id} onClick={() => setTapFlag(menu.value)}>{menu.title}</StyledButton>
                    ))
                }
            </TapMenu>
            <Process>
...
const StyledButton = styled.button`
  ...
  box-shadow: ${props => props.active ? "1px 3px 2px rgb(100 100 100 / 30%)": "none"};
